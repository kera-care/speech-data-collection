let promptHelper = require(Runtime.getFunctions()["messaging/send_prompt"].path);
let varsHelper = require(Runtime.getFunctions()["vars_helper"].path);
let transcriptionHelper = require(Runtime.getFunctions()["transcription"].path);

/**
 * Main entrypoint for Waxal workflow. See https://www.twilio.com/docs/serverless/functions-assets/functions
...
exports.handler = async (context, event, callback) => {
  ...
  try {
    ...
    let participant = participantRows.find((row) => row["Phone"] === participantPhone);
    ...
    if (participant) {
      ...
      if (status === "Prompted") {
        ...
        await handlePromptResponse(context, lastPrompt, event["Body"], event["MediaUrl0"], participant);
      ...
      if (participant["Status"] === "Completed") {
        ...
        await promptHelper.sendPrompt(context, participant["Phone"], "", surveyCompletedAudio);
      }
    } else {
    ...
    console.log(e);
    await promptHelper.sendPrompt(context, participantPhone, "", varsHelper.getVar("error-message-audio"));
  }
  callback(null, event);
};
 ...
 * @returns
 */
async function handlePromptResponse(context, lastPrompt, body, mediaUrl, participant) {
  if (participant["Type"] === "Transcriber") {
    ...
    await transcriptionHelper.addTranscription(participant["Key"], lastPrompt, body);
  } else {
    ...
    await uploadHelper.uploadVoice(context, lastPrompt, mediaUrl, participant);
  }
...
async function handleSendPrompt(context, participant) {
  ...
  let fetchedPrompt = isTranscription
    ? await transcriptionHelper.getNextPrompt(participant["Key"], participant["Language"])
  ...
  if (prompt["media"]) {
    ...
    await promptHelper.sendPrompt(context, participant["Phone"], positionString);
    await promptHelper.sendPrompt(context, participant["Phone"], "", prompt["media"]);
  } else {
